...
def docker_deploy(hosts,proj,tag,docker_run,docker_image_url,wait_times,exec_mode,log_file,check_time,task_id,serial,service_port,service_health_check_url):
    sklog = sklog_original(log_file)
    if service_port == "null":
        if exec_mode == "update":
            ansible_cmd = "ansible-playbook sc_update_hard.yml  --skip-tags 'healthcheck' -v -e \"hosts=%s DockerApp=%s DockerImageTag=%s DockerRun='%s' DockerImageURL=%s   TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % \
                        (hosts, proj, tag, docker_run, docker_image_url, task_id, serial,service_port,service_health_check_url,check_time)
        elif exec_mode == "update_hard":
            ansible_cmd = "ansible-playbook sc_update_hard.yml --skip-tags 'healthcheck' -v -e \"hosts=%s DockerApp=%s DockerImageTag=%s DockerRun='%s' DockerImageURL=%s TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % \
                            (hosts, proj, tag, docker_run, docker_image_url, task_id, serial,service_port,service_health_check_url,check_time)                  
        elif exec_mode == "restart":
            ansible_cmd = "ansible-playbook sc_restart_hard.yml  --skip-tags 'healthcheck' -v -e \"hosts=%s DockerApp=%s TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % (hosts, proj, task_id, serial,service_port,service_health_check_url,check_time)
                        
        elif exec_mode == "restart_hard":
            ansible_cmd = "ansible-playbook sc_restart_hard.yml --skip-tags 'healthcheck' -v -e \"hosts=%s DockerApp=%s TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % (hosts, proj, task_id, serial,service_port,service_health_check_url,check_time)
        elif exec_mode == "rollback":
            ansible_cmd = "ansible-playbook sc_rollback.yml --skip-tags healthcheck,manual -v  -e \"hosts=%s Serial=%s DockerApp=%s   WaitTimes=%s   TaskId=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % \
                            (hosts, serial,proj, wait_times,task_id,service_port,service_health_check_url,check_time)
        ...
        if exec_mode == "update":
            ansible_cmd = "ansible-playbook sc_update_hard.yml -v -e \"hosts=%s DockerApp=%s DockerImageTag=%s DockerRun='%s' DockerImageURL=%s   TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % \
                        (hosts, proj, tag, docker_run, docker_image_url, task_id, serial,service_port,service_health_check_url,check_time)
        elif exec_mode == "update_hard":
            ansible_cmd = "ansible-playbook sc_update_hard.yml --skip-tags 'healthcheck' -v -e \"hosts=%s DockerApp=%s DockerImageTag=%s DockerRun='%s' DockerImageURL=%s TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % \
                            (hosts, proj, tag, docker_run, docker_image_url, task_id, serial,service_port,service_health_check_url,check_time)                  
        elif exec_mode == "restart":
            ansible_cmd = "ansible-playbook sc_restart_hard.yml  -v -e \"hosts=%s DockerApp=%s TaskId=%s Serial=%s ServicePort=%s HealthCheckUrl=%s MaxCheckTime=%s\" " % (hosts, proj, task_id, serial,service_port,service_health_check_url,check_time)
...
def main(argv):
    ...
    service_port = load_pri_json_conf(CONFIG_BASE_DIR,env, proj)["ServicePort"]
    if service_port == "none"  or service_port == "":
        service_port = "null"

    service_health_check_url = load_pri_json_conf(CONFIG_BASE_DIR,env, proj)["HealthCheckUrl"]
    ...
    docker_deploy(hosts,proj,tag,docker_run,docker_image_url,wait_times,exec_mode,log_file,check_time,task_id,serial,service_port,service_health_check_url)
