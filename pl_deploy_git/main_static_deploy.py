...
def static_deploy(change_owner_tag,hosts,deploy_src_path,deploy_dest_path,delete_enable,owner,group,log_file):
    ...
    if hosts in ["web-site","web-cms-app"]:
        ansible_cmd = "ansible-playbook sc_static_sync.yml --tags php_frontend,%s -e \"hosts=%s deploy_src_path=%s  deploy_dest_path=%s delete_enable=%s owner=%s group=%s\" " % \
                  (change_owner_tag,hosts, deploy_src_path, deploy_dest_path, delete_enable,owner,group)
    else:
        ansible_cmd = "ansible-playbook sc_static_sync.yml --tags common,%s -e \"hosts=%s deploy_src_path=%s  deploy_dest_path=%s delete_enable=%s owner=%s group=%s\" " % \
                      (change_owner_tag,hosts, deploy_src_path, deploy_dest_path, delete_enable, owner, group)
    try:
